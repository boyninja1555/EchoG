...
def main():
    if len(sys.argv) > 1:
        open_browser_window(send_http_request("echog", sys.argv[1], None)["body"])
    else:
        open_browser_window(send_http_request("echog", "search.echog", None)["body"])
...
def open_url(url):
    if url.startswith("http://") or url.startswith("https://"):
        ...
        if len(url_parts) == 1:
            open_page("text/html", send_http_request("echog", url_parts[0], None)["body"])
        elif len(url_parts) == 2:
            open_page("text/html", send_http_request("echog", url_parts[0], url_parts[1])["body"])
...
def open_browser_window(page_contents):
    ...
    url_bar_entry.bind("<Return>", lambda event: open_page("text/html", send_http_request("echog", url_bar_entry.get(), None)["body"]))
    url_bar_entry.focus()
