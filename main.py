...
def main():
    if len(sys.argv) > 1:
        open_browser_window(send_http_request("EchoG", sys.argv[1], None).get("body"))
    else:
        open_browser_window(send_http_request("EchoG", "search.echog", None).get("body"))

def open_url(url):
    if url.startswith("http://") or url.startswith("https://"):
        webbrowser.open(url)
    elif url.startswith("echog://"):
        url_parts = url.split("echog://")[1].split("/")

        if len(url_parts) == 1:
            open_page("text/html", send_http_request("EchoG", url_parts[0], None).get("body"))
        elif len(url_parts) == 2:
            open_page("text/html", send_http_request("EchoG", url_parts[0], url_parts[1]).get("body"))
...
def open_browser_window(page_contents):
    ...
    url_bar_entry.bind("<Return>", lambda event: open_page("text/html", send_http_request("echog", url_bar_entry.get(), None).get("body")))
    url_bar_entry.focus()
...
def open_page(mime_type, page_contents):
    ...
    if mime_type == "text/plain":
        ...
        window.title(f"{url_bar_entry.get()} - EchoG")
    elif mime_type == "text/html":
        html_viewer = tk.Frame(viewer_frame, background="#222222")
        html_viewer.pack(fill=tk.BOTH, expand=True)

        window.title(f"{page_contents.split("<title>")[1].split("</title>")[0]} - EchoG")
